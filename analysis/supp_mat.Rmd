...
We can now predict values of $D$ ($D_p$) for all experiments 2x and 4x (referred to as $D_c,s$ in the main manuscript).
...
```
This should match Figure 4 (bottom right) in Buetti et al (2019). As a final check that we correctly understand the TCS model and have repimplmented it correctly, we can calculate the $R^2$ between predicted and empirical reaction times. 
...
```{r model_diagnostics, fig.height = 5}
