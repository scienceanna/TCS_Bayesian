...
Dp_samples <- map_df(c("2a", "2b", "2c", "4a", "4b", "4c"), get_Dp_samples, d)
...
names(beta_err) <- str_remove(names(beta_err), 'Dp:method')
 ...
 beta_err %>% pivot_longer(1:3, names_to = "method", values_to = "abs_res") %>%
 ...
 beta_err %>% select(-best_feature) %>%
  ...
  beta_err %>% select(-best_feature) %>%
   mutate(difference = orthog_contrast - collinear) -> x
