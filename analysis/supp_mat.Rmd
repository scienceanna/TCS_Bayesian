...
```{r}
VarCorr(m_exp1_log)$p_id$sd
VarCorr(m_exp2_log)$p_id$sd
```

Residual variance looks similar, although exp2 is lower that exp 1. 

```{r}
VarCorr(m_exp1_log)$residual$sd
VarCorr(m_exp2_log)$residual$sd
```
...
# 
# mdl_inputs_log <- set_up_model(1, "lognormal")
# power_model_log <- run_model(mdl_inputs_log, ppc = "no")
#   
# plot_model_fits_rt(1, power_model_log, plot_type = "fitted")
```
