...
if (refit_priors) {
  ...
  saveRDS(prior_model_nrl, "models/exp_1_nrl.models")
  
  mdl_inputs_log <- set_up_model(1, "lognormal")
  prior_model_log <- run_model(mdl_inputs_log, ppc = "only")
  saveRDS(prior_model_log, "models/exp_1_log.models")
  
  mdl_inputs_sft <- set_up_model(1, "shifted_lognormal")
  prior_model_sft <- run_model(mdl_inputs_sft, ppc = "only")
  saveRDS(prior_model_sft, "models/exp_1_sft.models")
  
} else {
  
  prior_model_nrl <- readRDS("models/exp_1_nrl.models")
  prior_model_log <- readRDS("models/exp_1_log.models")
  prior_model_sft <- readRDS("models/exp_1_sft.models")
}

```

```{r plt-prior, fig.height=16, fig.cap="Sample Prior predictions for reaction time and log(N+1)."}

plt_nrl <- plot_model_fits_rt(1, prior_model_nrl, y_limits = c(-2, 10))
plt_log <- plot_model_fits_rt(1, prior_model_log, y_limits = c(0, 10))
plt_sft <- plot_model_fits_rt(1, prior_model_sft, y_limits = c(0, 10))
...
```{r plt-post, fig.height=16, fig.cap="Posterior predictions for reaction time and log(N+1)."}
...
```{r plt-post-re, fig.height=16, fig.cap="Posterior predictions for reaction time and log(N+1)."}

plt_nrl <- plot_model_fits_rt(1, m_exp1_nrl, plot_type = "predicted", y_limits = c(0, 4))
plt_log <- plot_model_fits_rt(1, m_exp1_log, plot_type = "predicted", y_limits = c(0, 4))
plt_sft <- plot_model_fits_rt(1, m_exp1_sft, plot_type = "predicted", y_limits = c(0, 4))
...
ggplot(slopes1, aes(x= D, fill = d_feature)) + geom_density(alpha = 0.5)
...
Dp_samples %>% 
  ...
  mean_hdci(Dp, De) %>%
  ggplot(aes(x = Dp, y = De),  color = "yellow1") +
  geom_abline(linetype = 2) +
  geom_point(color = "yellow1") +
...
model_params <- set_up_predict_model(2, "lognormal", meth, Dp_summary)
...
```{r plot-exp2-predictions, fig.height = 8, fig.cap="reaction time predictions."}
plot_model_fits_rt(2, m_prt, y_limits = c(0, 2.5), n_row = 3, plot_type = "fitted")
```

Attempting plots for unknown participant! 

```{r plot-exp2-predictions-new-person, fig.height = 8, fig.cap="reaction time predictions."}
plot_model_fits_rt(2, m_prt, y_limits = c(0, 2.5), n_row = 3, plot_type = "predicted")
```
