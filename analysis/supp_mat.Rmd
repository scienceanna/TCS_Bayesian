...
```{r pred-rt-replication, fig.width = 4.5, fig.asp=1, fig.cap="Computational Replication of RT predictions."}
...
```
This should match Figure 4 (bottom right) in Buetti et al (2019). As a final check that we correctly understand the TCS model and have repimplmented it correctly, we can calcualte the $R^2$ between predicted and empirical reaction times. 
...
```{r plt-prior, fig.height=2.5, fig.cap="Sample Prior predictions for reaction time and log(N+1)."}
...
```{r plt-post, fig.height=10, fig.cap="Posterior predictions for reaction time and log(N+1)."}
...
```{r plt-post-re, fig.height=10, fig.cap="Posterior predictions for reaction time and log(N+1)."}
...
tibble(model = c("normal", "lognormal", "shifted-lognormal", "linear in N_d"),
...
We will need to use the `bridge_sampler()` function later on, so let's use it here and verify that it gives the same results as `loo()`.
...
Now we can compute $D$ by extracting the fixed effect slopes from the model for each experiment and condition.
...
Now, we use the model fitted on experiment 1 (or 3) to predict the data in experiment 2 (or 4). I.e., can we predict $D$ for compound features based on the $D$$ values for the related single features. 
...
m_prt_linear_transform <- compute_rt_predictions(meth = "linear_transform")
...
bs_fit <- bridge_sampler(m_exp2_sft, silent = TRUE)
post_prob(bs_fit, bs_bfeat,bs_col, bs_orth)
...
mp <- readRDS("models/exp_1p_sft.models")
d<- dr_shuffle 
plot_model_fits_rt(1, mp, plot_type = "fitted")

slopes1 <- extract_fixed_slopes_from_model(mp)
  ...
  bs_exp1p_sft <- bridge_sampler(mp)  
    post_prob(bs_exp1_sft, bs_exp1p_sft)
