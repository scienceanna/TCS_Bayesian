...
left_join(Dp, De, by = c("exp_id", "d_feature")) %>%
  ...
  geom_smooth(method = "lm", formula = y ~ 0 + x, colour = "violetred3") + 
...
```
The $R^2=$ `r r2`. However, this is potentially misleading, as it doesn't show how the model fits the different conditions! 


```{r fig.height = 6, fig.cap="Breaking down the TCS predictions by condition."}
...
rt_pred %>%
  ...
  geom_point(data = d_mean_mean, aes(y = mean_rt), colour = "yellow1" )
...
rm(De, Dp, rt_pred, exps_to_predict, 
   calc_D_per_feature, extract_a_value, predict_D_overall, gen_exp_predictions, extract_D, predict_rt, d_corr, r2)
```
...
d_r2 <- tibble(method = as.character(),
               experiment = as.numeric(),
               r2_lower = as.numeric(),
               r2_upper = as.numeric())

for (meth in unique(Dp_samples$method)) {
  df <- filter(Dp_samples, exp_id == 2, method == meth)
  r2 <- map_dbl(unique(df$iter), get_D_R2) %>% hdci(.width = 0.97)
  d_r2 <- add_row(d_r2, method = meth, experiment = 2, r2_lower = r2[1], r2_upper = r2[2])
  
  df <- filter(Dp_samples, exp_id == 4, method == meth)
  r2 <- map_dbl(unique(df$iter), get_D_R2) %>% hdci(.width = 0.97)
  d_r2 <- add_row(d_r2, method = meth, experiment = 4, r2_lower = r2[1], r2_upper = r2[2])
...
```{r plot-exp2-predictions-new-person, fig.height = 6, fig.cap="reaction time predictions.", cache = TRUE}
