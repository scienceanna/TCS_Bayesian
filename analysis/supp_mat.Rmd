...
```{r predict-D-2, fig.cap = "Estimating D."}
...
lm_D <- function(df) {
  ...
  return(tibble(method = unique(df$method), 
...
bind_rows(Dp_samples2, Dp_samples4) %>% 
  group_by(iter) -> Dp_samples


group_map(Dp_samples, ~ lm_D) -> a


rm(
  slopes1, slopes2, slopes3, slopes4, 
  Dp_samples2, Dp_samples4)
...
plot_model_fits_rt(2, m_prt, y_limits = c(0, 1.25), n_row = 3, plot_type = "fitted")
```
