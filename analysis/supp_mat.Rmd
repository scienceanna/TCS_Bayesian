...
(There's an additional model here for the shifted lognormal, using the model formula d_feature:(N_T + 1) rather than d_feature:log(N_T + 1))
...
bs_exp1_nrl <- bridge_sampler(m_exp1_nrl)
bs_exp1_log <- bridge_sampler(m_exp1_log)
bs_exp1_sft <- bridge_sampler(m_exp1_sft)
bs_exp1_sft_nolog <- bridge_sampler(m_exp1_sft_nolog)

post_prob(bs_exp1_nrl, bs_exp1_log, bs_exp1_sft, bs_exp1_sft_nolog)
...
Dp_samples %>% pivot_wider(c(exp_id, d_feature, iter, De), names_from = method, values_from = Dp) -> d_lmer

m_lmer <- lme4::lmer(data = d_lmer, De ~ best_feature + orthog_contrast + collinear + (1|iter))
d_lmer %>% 
  mutate(linear_transform = predict(m_lmer)) %>%
  pivot_longer(
    c(best_feature, orthog_contrast, collinear, linear_transform), 
    names_to = "method", 
    values_to = "Dp") -> Dp_samples
...
bs_bfeat <- bridge_sampler(m_prt_best_feature)
bs_orth <- bridge_sampler(m_prt_orthog_contrast)
bs_col <- bridge_sampler(m_prt_collinear)
...
bs_fit <- bridge_sampler(m_exp2_sft)
