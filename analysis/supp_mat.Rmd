...
```{r rt_exp-1b-hist, fig.cap="Histogram showing indivdidual differences in median RT in Experiment 1b."}
...
mdl_inputs <- set_up_model("1", d, "normal")
prior_model_nrl <- run_model(mdl_inputs, ppc = "only")

mdl_inputs <- set_up_model("1", d, "lognormal_shifted")
prior_model_log <- run_model(mdl_inputs, ppc = "only")

mdl_inputs <- set_up_model("1", d, "shifted")
prior_model_sft <- run_model(mdl_inputs, ppc = "only")
```

```{r plt-prior, fig.height=8, fig.cap="Sample Prior predictions for reaction time and log(N+1)."}
plt_nrl <- plot_model_fits_rt(d, 1, prior_model_nrl)
plt_log <- plot_model_fits_rt(d, 1, prior_model_log)
plt_sft <- plot_model_fits_rt(d, 1, prior_model_sft)
...
if (refit_models)
{
  ...
  saveRDS(m_exp1_log, "exp_1_log_AH.models")
  
  m_exp1_sft <- fit_glmm_to_an_exp(1, d, ppc = "no", fam = "shifted_lognormal")
  saveRDS(m_exp1_sft, "exp_1_sft.models")
  
} else {
  
  m_exp1_nrl <- readRDS("exp_1_nrl.models")
  m_exp1_log <- readRDS("exp_1_log_AH.models")
...
if (refit_models)
{
  ...
  m_exp3_log <- readRDS("exp_3_log.models")
  m_exp4_log <- readRDS("exp_4_log.models")
}
```
...
d_prt <-  run_model(model_params, ppc = "only")
