...
```{r glmm-prior-sample, cache=TRUE, fig.cap="Sample Prior predictions for reaction time and log(N+1)."}
prior_model_nrl <- fit_glmm_to_an_exp("1a", d, ppc = TRUE, "normal")
prior_model_idt <- fit_glmm_to_an_exp("1a", d, ppc = TRUE, "lognormal")
prior_model_sft <- fit_glmm_to_an_exp("1a", d, ppc = TRUE, "shifted")
#prior_model_gam <- fit_glmm_to_an_exp("1a", d, ppc = TRUE, "Gamma")
```

```{r}
plt_nrl <- plot_model_fits_ex(d, "1a", prior_model_nrl, 1)
plt_idt <- plot_model_fits_ex(d, "1a", prior_model_idt, 1)
plt_sft <- plot_model_fits_ex(d, "1a", prior_model_sft, 1)
#plt_gam <- plot_model_fits_ex(d, "1a", prior_model_gam, 1)
