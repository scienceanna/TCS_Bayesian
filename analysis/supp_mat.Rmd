...
De %>% group_by(exp_id, d_feature) %>% 
  mean_hdi(D) %>%
  rename(De_lower = ".lower", De_upper = ".upper", De = D) %>% 
  right_join(Dp, by = c("exp_id", "d_feature")) %>%
  select(-.width, -.point, -.interval) -> d_D 

ggplot(d_D, aes(x = Dp, y = De)) + 
  geom_abline(linetype = 2) + 
  geom_linerange(aes(ymin = De_lower, ymax = De_upper), alpha = 0.5) + 
  geom_linerange(aes( xmin = Dp_lower, xmax = Dp_lower), alpha = 0.5) + 
