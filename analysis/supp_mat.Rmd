...
Calculate $D$ for each condition in each experiment. 
...
```{r plot-glm-to-buetti2019-data, cache=TRUE, fig.cap="The multi-level fits for Experiment 1a (Buetti, 2019)."}
plot_model_fits_ex(d, "1a", my_models[[1]], 1:5)
```
...
```{r}
# Predict experiments
pred_D <- map_df(c("2a", "2b", "2c", "4a", "4b", "4c"), gen_exp_predictions_b, d)
...
rt_pred <- map_dfr(exps2predict, predict_rt_b)

ggplot(rt_pred, aes(x = N_T, y = median_rt, ymin = lower, ymax = upper, fill = d_feature, colour = d_feature)) + 
  geom_path() + 
  geom_ribbon(alpha = 0.5, colour = "black") + 
  facet_grid(method ~ exp_id) + 
  see::theme_abyss()
```
