...
# m_exp1_log <- add_criterion(m_exp1_log, c("loo"))
# m_exp1_sft <- add_criterion(m_exp1_sft, c("loo"))
...
```{r}
# model_weights(m_exp1_nrl, m_exp1_log, m_exp1_sft)
```
...
### Use extract $D$ values for Experiment 1
...
ggsave("../plots/buetti2019_bayesian_fits_exp1.pdf", width = 8, height = 3)
```
...
Dp_samples2 <- get_Dp_samples(2, d, slopes2)
...
Dp_samples2 %>%
  ...
  geom_smooth(method = lm, colour = "pink") -> plt2
...
Dp_samples4 <- get_Dp_samples(2, d, slopes2)
...
Dp_samples4 %>%
  ...
  geom_linerange(aes(ymin = De.lower, ymax = De.upper), alpha = 0.5) +
  geom_linerange(aes(xmin = Dp.lower, xmax = Dp.lower), alpha = 0.5) +
  geom_smooth(method = lm, colour = "pink") -> plt4

ggsave("../plots/recreate_log_normal_fig_4_4.pdf", width = 8, height = 2.5)

```

```{r}
plt2 / plt4
ggsave("../plots/recreate_log_normal_fig_4.pdf", width = 8, height = 4)
rm(plt2, plt4)
...
# d_prt <- predict_rt_b(2, meth, Dp_summary, d)
```
