...
m_prt_collinear <- run_model(model_params, ppc = "only")
...
model_params <- set_up_predict_model("2", "shifted_lognormal", meth, Dp_summary, m_exp1_sft, m_exp2_sft)
m_prt_orthog_contrast <- run_model(model_params, ppc = "only")
# 
# model_weights(m_prt_collinear, m_prt_orthog_contrast)
# 
# loo_collinear <- loo(m_prt_collinear, nsamples=500)
# loo_orthog_contrast <- loo(m_prt_orthog_contrast, nsamples=500)

bs_orth <- bridge_sampler(m_prt_orthog_contrast)
bs_col <- bridge_sampler(m_prt_collinear)
post_prob(bs_col, bs_orth)
