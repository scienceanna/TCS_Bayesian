...
df_r2 <- tibble(exp_id = as.numeric(), method = as.character(), r2 = as.numeric())

for (e_id in c(2,4)) {
  for (meth in unique(Dp_tmp$method)) {
    df <- filter(Dp_tmp, method == meth, exp_id == e_id)
    r2 <- summary(lm(D ~ 0 + Dp, data = df))$r.squared
    
    df_r2 <- add_row(df_r2, 
                      exp_id = e_id, method = meth, r2 = r2)
  }  

}

knitr::kable(df_r2, digits =3)
...
Compute correlation!
```{r}
d %>% filter(exp_id %in% exps2predict, N_T > 0) %>%
	group_by(exp_id, p_id, d_feature, N_T) %>%
	summarise(mean_rt = mean(rt), .groups = "drop") %>%
	group_by(exp_id,  d_feature, N_T) %>%
	summarise(mean_rt = mean(mean_rt), .groups = "drop") %>%
