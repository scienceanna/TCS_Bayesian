...
saveRDS(m, "pilot2_sensitivity_fixed.model")
...
samples1 <- get_slopes(m, 1) %>% select(-observer, -rD) %>% distinct() %>%
...
samples2 <- get_slopes(m, 1) %>% select(-observer, -rD) %>% distinct() %>%
   filter(str_detect(feature, '_')) 
...
```{r, cache=TRUE, message = FALSE, warning = FALSE, echo = FALSE, eval = FALSE}
  d2 <- d2 %>% unite(feature, feature1, feature2)
