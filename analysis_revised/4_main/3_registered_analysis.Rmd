...
`../scripts/our_functions.R` contains functions that ...
...
samples1 <- extract_samples_from_model(m1)
...
samples2 <- extract_samples_from_model(m2)
...
calc_D <- function(feature1, feature2) {
 ...
 return(tibble(.draw = samples1$.draw,
...
```{r, cache=FALSE, message = FALSE, warning = FALSE, echo = FALSE, fig.height = 5, , fig.cap = "Predicted RTs for our pilot data, using collinear contrast combination."} 
...
``` 
 ...
 With the pilot data, all three models receive approximately the same predictive weight, meaning that we are not able to distinguish etween them: we do not have evidence for one model over the others.  
