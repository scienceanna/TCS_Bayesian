...
```{r plot_single_feature_corr, warning = FALSE, message = FALSE}
plt_D_obs + plt_corr
...
```{r plot_cor_double_feature, fig.height=6, warning = FALSE}
...
```{r plot_Dp, fig.height = 15, message = FALSE}
ggplot(slopes, aes(Dp, De, colour = method)) + geom_point() +
  geom_smooth(method = lm, alpha = 0.25) +
  geom_abline(linetype = 2) + 
  facet_wrap(~observer, nrow = 10)
```
