...
First, we will look to see the extent to which $D$ varies from one person to the next. And are these variations correlated? 
...
```{r plot_single_feature_corr}
plt_D_obs + plt_corr

ggsave("../../plots/single_feature_correlations.pdf", height = 3, width = 8)

rm(plt_Dobs, plt_corr)
```

## Now compute correlations for double feature search 

```{r compute_double_feature_corrcache=TRUE, cache=TRUE}
...
```{r plot_cor_double_feature, fig.height=6}
