...
dp %>% 
  filter(lnd > 0) %>%
  group_by(.draw,lnd, observer, ring, method) %>%
...
Derr %>% ungroup() %>%
  group_by(method) %>% 
  median_hdi(rel_mean_abs_err, .width = 0.97) %>%
  knitr::kable() %>%
  kable_styling() 
...
dp %>% 
  filter(lnd > 0) %>%
  group_by(.draw,lnd, ring, method, observer) %>%
  summarise(llh = mean(loglik), .groups = "drop") %>%
  pivot_wider(names_from = "method", values_from = "llh") %>%
  pivot_longer(c(D_collinear, D_best_feature, D_orth_contrast), 
               names_to = "method", values_to = "Dp") %>%
  mutate(rel_llh = exp(Dp-D)) -> Dllh

Dllh %>% ungroup() %>%
  group_by(method) %>% 
  median_hdi(rel_llh, .width = 0.97) %>%
  knitr::kable()
