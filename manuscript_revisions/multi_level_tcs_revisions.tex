...
\end{tabular}
\caption{A table of $D_i$ values for Experiment 1a and 1b. See Supplementary Material for full values for all experiments.}
...
It is also worth noting that relying simply on $R^2$ values may give a slightly misleading view of the accuracy of the TCS model. To some extent, this could reflect the fact that the $R^2$ measure of goodness-of-fit simply measures how tightly clustered the points are around some line with arbitrary intercept and slope. It is possible therefore for the $R^2$ value to be high, but for the line to deviate significantly from an intercept of 0 and a slope of 1, which is the unity line required for the predictions to be a good match to the empirical data. For example, Figure 2.2 in the supplementary material shows that the best feature model has a relatively high $R^2$ value (0.869) but the best fit line deviates significantly from $y = x$. In \cite{buetti2019predicting}, a mean absolute error metric was used to assess this aspect of model fit, and was found to be in agreement with the $R^2$ metric: however, this does not necessarily hold in general. For our reimplementation of the TCS model, we will therefore assess model fit by computing the log marginal likelihoods via bridge sampling. This directly assesses how well our values of $D_{c,s}$ are able to predict RTs, avoiding the issues with the $R^2$ measure.

Figure \ref{fig:comp_rep_conditions} shows the predicted RTs (pink line) for the conditions from Experiment 2 (a, b and c) of \cite{buetti2019predicting}, plotted next to the mean of means for each condition. It is clear that for at least some conditions, the TCS model (using the collinear method) is not predicting the empirical data well, even when allowing quite rigid constraints on the model; for example, the intercept, $a$, is derived from the data for each sub-experiment (e.g. Experiment 2a) and thus will always reflect the observed, true mean for $N_T = 0$ for that dataset (rather than being predicted from the data in Experiment 1). 

Finally, if we look at the different predictions made by the three proposed feature combination models, we can see that in most of the parameter space explored in \cite{buetti2019predicting}, the models all make very similar predictions (Figure \ref{fig:comp_rep_conditions2}). In Experiment 2, we are only able to discriminate between models in the \textit{blue} condition; in the \textit{yellow} and \textit{orange} conditions, they make essentially identical predictions. As such, if we want to distinguish between these three models, we will have to use combinations of features that lead to larger target-distractor differences. 

\begin{figure}
\centering
\includegraphics[width=\textwidth]{../plots/computational_replication_issues.pdf}
\caption{(left) Conditions from Experiment 2. Blue dots represent the mean of means for each condition, while the line shows the prediction of the TCS theory (for the collinear combination). (right) As before, but conditions from Experiment 4.}
\label{fig:comp_rep_conditions}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\textwidth]{../plots/computational_replication_issues2.pdf}
\caption{ Estimates of $D_{c,s}$ for the different colours and shape from Experiment 2 in \cite{buetti2019predicting}. Note that both axes are continuous (labels on the x axis are included as a convenience for the reader, so they are able to see which $D_s$ value corresponds with each shape). As can be seen, in two of the three colour conditions, we are unable to distinguish between best feature, collinear or orthogonal contrast models.}
\label{fig:comp_rep_conditions2}
\end{figure}
...
However, switching from linear regression to a multi-level model raises the problem of which distribution to use for modelling reaction times. Using a normal distribution is unlikely to be satisfactory, as it is unable to account for the skew frequently seen in reaction time distributions, and also allows the possibility of negative reaction times. We can account for both of these problems by using a \textit{lognormal} distribution, $\text{rt} \sim \text{lognormal}(\mu, \sigma)$. We will also test whether a slightly more complex extension of this model, the \textit{shifted lognormal} model (which allows the distribution to be offset to the right i.e. mimicking the patterns seen in reaction time data, where valid responses begin at around 100ms) offers any improvement in model fit.\footnote{Note that a Wald, or inverse Gaussian distribution, would also be a reasonable distribution choice for this data given that TCS is based on a diffusion process e.g. \cite{moran2013competitive}: we chose not to use this distribution as it often leads to computational issues, which would make it harder for others to reproduce or build on our approach later.}
...
The $R^2$ values for the four approaches are shown in Table \ref{tab:reimp_D_r2}. We can see that as with the original model, we still find the best feature model is the worst of the three methods, at least in terms of the raw $R^2$ value. However, it is less clear which of the other methods gives the best performance. While the collinear contrast approach offers the highest $R^2$ (HPDI: 0.827-0.965), it consistently predicts values that are too low (Figure \ref{fig:buetti2019_D}, bottom). The orthogonal contrast method gives more accurate predictions, albeit with a slightly lower $R^2$ (HPDI: 0.748-0.949). In order to distinguish between the different methods, we instead compare goodness-of-fit of the different models for predicting reaction times (see below).
...
\caption{(\textit{left}) HPDI and empirical mean reaction times for each condition in Experiment 2, using the Bayesian multi-level modelling approach and the collinear contrast integration model to predict D values. (\textit{right}) Similar, for the full distribution of reaction times over trials. }
...
Our extension to a multi-level TCS model allows us to go beyond the original version and predict the full distribution of reaction times over samples of known, or new, observers. Moving from a normal to a shifted-lognormal distribution allows us to accurately model the skew seen in the empirical data, and avoid predicting negative response times. Our modelling has also highlighted other issues that we will address during our replication study. 

Firstly, we will use $a$ from the single feature experiments (i.e. Experiment 1) to predict $a$ in the compound features experiments (i.e. Experiment 2), as we plan to run our experiments as a within-subject design, and thus we would not expect systematic differences in non-search processes due to individual differences, as might be seen in a between-subjects design, such as that employed in \citep{buetti2019predicting}.

Secondly, we will choose distractor features to give the highest possible $D$ sensitivity, and thus give us greater power to detect true differences between models.

Finally, the multi-level model is also easily extended to incorporate extra factors which may affect RTs on a trial-by-trial basis e.g. which ring the target is in. We demonstrate an example of extending the model in this way in the supplementary material.
...
\\
\item \textbf{Increase target-distractor similarity.} Our reanalysis of \cite{buetti2019predicting} indicates that the orange distractor condition does not distinguish well between different contrast models. We will therefore redesign the target and distractors, guided by pilot data, in order to better. distinguish between the contrast models 
\end{enumerate}
...
We will also run exploratory analyses including a factor for which ring the target is in to assess whether this improves model fit or affects any of the conclusions that can be drawn from the model.

\section{General Methods}

\subsection{Sample Size: Participants and Trials}

To determine our sample size, we carried out a simulation study to estimate the effect of reducing the number of trials and increasing the number of participants on our ability to accurately measure the $D_i$. While our reanalysis of \cite{buetti2019predicting}'s data is unable to distinguish between the three models of contrast combination, we believe that this is due to the feature values used in their study (see above and Figure \ref{fig:comp_rep_conditions}). Therefore, we will base our sensitivity analysis around the width of the $97\%$ HPDI when estimating $D_{\text{blue}}$. We have chosen to look at this single feature level as the blue distractor trials offer the greatest discrimination between the three contrast models.
...
\textbf{Distractor and target types:} we will replicate the distractor types used in \cite{buetti2019predicting}, apart from that we will change the distractor colours to allow us to discriminate better between different models of the data (see above). There are six single-feature conditions (yellow, orange and purple distractors and triangle, circle and diamond distractors) and nine double-feature conditions (all possible pairings of the single-feature conditions). The target is always a red semicircle, except in the trials where the distractors are single-feature shapes (triangles, circles and diamonds) in which case the target is a white semicircle.

\textbf{Set sizes:} we will run all the distractor set sizes used in \cite{buetti2019predicting} (1, 4, 9, 19 and 31). We will also run target-only 'zero distractor' trials (30 in total, with 6 being the white semicircle target and the remainder the red semicircle target).
...
Participants will view a fixation cross before viewing a search array: they will press the space bar to continue to the trial. Participants will be told to search for the target among distractors (i.e. will be told to perform a singleton search) and report if the semicircle target points to the left or right, by pressing either the `f' or `j' key respectively on their keyboard. They will first complete 20 practice trials where they will receive feedback immediately after completing each trial. In the real experimental trials, participants will receive feedback on their average accuracy and reaction time after each block of 30 trials. Participants will complete 19 blocks of trials (570 trials overall i.e. 114 trials in each of 5 experiments, consisting of 6 set sizes x 3 distractor conditions x 6 repeats + 6 zero distractor trials) with the order of the stimuli being fully randomised i.e. all different conditions will be completely intermixed. (This is different from \cite{buetti2019predicting}: they ran each sub-experiment on separate participants, whereas we will run all the participants on all sub-experiments, in a non-blocked design i.e. the order of trials will be completely randomised across all sub-experiments).

In both the practice and experimental trials, the search display will always remain on screen until a response is made, or until 5 seconds have passed. 
