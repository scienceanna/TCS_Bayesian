...
\item \textbf{Contrast model comparisons.} We will test the hypothesis proposed by \citep{buetti2019predicting}: specifically, that the \textit{collinear contrast ingratiation model} outperforms the \textit{best feature guidance}, and \textit{orthogonal contrast combination models} for the calculation of $D$, by calculating and comparing the mean absolute prediction error for each model. \\
...
Full details of a pilot experiment with $n=4$ participants (960 trials each) can be found in supplementary materials. This suggests that even with a small sample, we can convincingly demonstrate H1 and H2, however more data will be required to discriminate between the models in H3. 
