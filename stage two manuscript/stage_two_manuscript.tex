...
Given these results, it is perhaps unsuprsing that our analysis for Hypothesis 4 leads to a neutral result between the three contrast combination methods. Perhaps taking these individaul differences into account when we predict the RTs will lead to an improved power to discriminate between the models. 
...
We have two main conclusions from our registered analysis. Firstly, linear models of response times in visual search in terms of set size should be not be used, at least in paradigms similar to the one presented here. The use of a shifted-lognormal distribution of response times and a loglinear effect of set size appears to be unambiguously better. 


\textit{say something about correaltions/. can cite \cite{clarke2020stable}}
