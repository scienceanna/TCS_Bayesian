...
All analysis will be carried out using R (v4.2.0), brms (v2.17.0) and rStan (v2.26.11). As discussed above, we will use a mixed-effect models with either normal, lognormal or shifted lognormal distributions. 
...
All Bayesian models were fit to thw new data using exactly the same procedure\footnote{the only departure was an increase in samples in the bridgesampling procedure from xxxx to xxxx. Based on advice given in the Stan forums} as the pilot data presented in the Stage One review process. All models converged with WHAT CRITERIA DO WE WANT TO SAY HERE? $\hat{R}$

\subsubsection{Hypothesis 1: Shifted-lognormal model}
...
a i) normal, ii) lognormal, and iii) shifted-lognormal distrubtion. The models all used the same model formula that estimated search slopes in terms of $\log{N_t}$ for each feature. Maximal random effect structures where used. 

After each of these models had been fit to the data, leave-one-out model comparison was used to calculate posterior probabilities for each. The results of this procedure are shown in Table \ref{tab:h12} and we can conclude that the shifted-lognormal is indeed the best distrubtion (out of the three we tested) to use for these models. This model is shown in Figure \ref{fig:sftln_sf}.

\begin{table}[]
\centering
\begin{tabular}{l|ll}
\textbf{model}             & \textbf{LOO} & \textbf{WAIC} \\
\hline
shifted-lognormal & 1   & 1    \\
lognormal         & 0   & 0    \\
normal            & 0   & 0   \\
\hline
linear in $\log{N_T}$ & 1   & 1    \\
linear in $N_T$         & 0   & 0    \\ 
\end{tabular}
\caption{Model comparison results for Hypotheses 1 and 2.}
\label{tab:h12}
\end{table}
...
\centering
%\includegraphics[width=\textwidth]{../plots/n_trials.pdf}
\caption{The shifted-lognormal model fits for the single-feature data. }
\label{fig:sftln_sf}
\end{figure}

\subsubsection{Hypothesis 2: log-linear effect of $N_T$}

We then used the same methods to verify that using $\log{N_T}$ for the search slope does indeed give a better fit to the data than simply using $N_T$. The results are shown in Table \ref{tab:h12} and again, our hypothesis is 

\subsubsection{Summary of search slopes }

Now that we have confirmed that the shifted-lognormal multilevel model (with a log-linear effect of $N_T$) is indeed the best fit to the data we will extract the search slopes for each feature. Tese are summarised in Table \ref{tab:new_Dc_Ds}

feature D   .lower  .upper
...
feature & $D_c$ & $95\%$HDCI \\
 \hline 
orange & 0.161 &  [0.144 ,  1.110]\\
pink  &  0.039 &  [-0.061 , 0.053]\\
purple & 0.019 &  [0.005  , 0.537]\\
 \hline 
  & $D_c$ &  \\
 \hline 
 circle & 0.188  & [-0.065 , 0.202]\\
diamond& 0.190 &  [0.174 ,  1.168]\\
triangle  &  0.258  & [0.237  , 0.849]\\
\end{tabular}
\caption{A table of $D_c$ and $D_s$ values from our Experiment}
\label{tab:new_Dc_Ds}
\end{table}

\subsubsection{Hypothesis 3: Contrast Model Comaparison}


\subsubsection{Reaction Time Predictions}

\subsection{Exploratory Analysis}

Does taking target location into account help with the reaction time predictions? 
