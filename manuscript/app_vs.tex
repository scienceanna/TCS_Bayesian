...
As we are using a Bayesian framework, rather than $D$ representing the best fit in equation \ref{eq:computeDlm}, it is now represented as a probability distribution over possible values (see Figure \ref{fig:buetti2019_a1}). We can can then estimate $D$ in the double feature distractor cases using the three methods detailed above and compare the predicted values to the empirical values (see Figure \ref{fig:buetti2019_D}). We additionally calculated a fourth prediction method, the 'mean method' which simply takes the averages of the collinear contrast and orthogonal contrast models.
