...
\caption{(left) The collinear method for calculating $D$ offers a good prediction. (centre) Using the TSC to predict reaction times. (right) Each dot now represents a randomly sampled reation from an observer.}
\label{fig:comp_rep}
\end{figure}
...
As can be seen in  Figure \ref{fig:comp_rep}, these predictions are very close to the observed values. 
...
We can then extract the posterior disbritubions for the logarithmic slope parameters as before. The only difference is that i) we have the full posterior distribution for each $D_c$ amd $D_s$, rather than just the maximum likelihood fit, and ii) the units are now in $\frac{\log{rt}}{\log{N_T+1}}$
