...
\subsubsection{Calculating $D_j$: the logarithmic slope parameter}


<outline exp 1 here... search for a target that differs from distracters with a single feature> 


The TCS model is built around a log-linear model for reaction times:

\begin{equation}
\hat{RT} = D\log(N_T) + c
\end{equation}

I.e., reaction times increase lienarly with $\log(N_T)$, and the error distribution is assumed to be normal. 
...
First, we computed the $D$ values using a multi-level Bayesian model using a log-normal distribution (see Equation \ref{eq:computeDlm}). Using this method allows us to fit each individual's performance. This is important because it is clear there are between-subjects differences that are not captured in the original model. 
...
We fit the model three times, once using a log link\footnote{our preference, as this avoids predicting negative reaction times and helps to account for skew in the distribution}, and once using the identity link\footnote{makes this new version of the model more comparable to the original}. [ADD IN EXTRA STUFF ABOUT SHIFTED GAUSSIAN MODEL?]

Other minor changes included modelling reaction times on the seconds scale, rather than milliseconds (making numerical model fitting easier) and removing all RTs less than 25ms.
