...
In Experiment 1a of \cite{buetti2019predicting}, participants searched for a cyan semicircle target among blue, yellow or orange semicircular distractors i.e. they searched for a target that differed from the distractors by a textit{single feature} (colour). The experiment was then repeated (1b) using a different single feature (shape, with participants searching for the semicircular target within triangle, circle or diamond distractors). In Experiments 2a, 2b and 2c, participants again searched for a cyan semicircle, but this time, the distracters differed in both shape and colour. We will refer to these conditions as \textit{double features}. Note, unlike in standard conjunction searches, in this paradigm, the distracters are all identical (i.e, orange triangles). 

The \textit{Target Signal Contrast} theory allows us to predict the value of the logarithmic slope, $D_\text{overall}$, in this condition based on the  corresponding $D_j$ in the single feature seach experiments. 

\subsubsection{Calculating the logarithmic slope parameter, $D_i$}
\label{sec:fitting_D}

In both experiments, the number of distractors varied, allowing the data to be used to fit a log-linear model for reaction times, where reaction times increase linearly with $\log(N_T)$ (the log of the number of distractors) \ref{eq:loglin}. In the original model the error distribution was assumed to be normal. 
...
Thus the results of Experiment 1 were used to calculate the logarithmic slope, for each type of distractor. When colour varied, we will refer to $D_c$, for $c=1,2,3$. Similar for share ($D_s$), and the compound features ($D_{c,s}$). As $a$ represened the intercept in our model, it is indepand of both shape and colour, as it represented how long observers take to find a target when $N_T = 0$, i.e., there are no distracters.

\subsubsection{Estimating $D_{c,s}$, the logarithmic slope parameter for compound features}

The core of the idea in the TSC theory is that we estimate the logarthmic slope parameter for a double feature visual search from the slopes parameters in the two independant single feature searches. I.e., $D_{c,s} = f(D_c, D_s)$.
...
While the TCS theory offers a good prediction of search slopes and corresponding mean reaction times for compound feature search, there are two related limitations. Firstly, it is unable to account for individual differences between observers, only the changes to the sample average. Secondly, it cannot account for the distribution of reaction times over multiple trials. To address these issues, we propose a second version of the TCS that makes use of mutli-level modelling techniques
...
However, switching from linear regression to a mutli-level model raises the problem of which disribution to use for modelling reaction times. Using a normal distribution is unlikely to be satisfactory, as it is unable to account for the skew frequently seen in rt distributions, and also allows the posibility of negative reaction times! We can account for both of these problems by using a log-normal distribution, $\text{rt} ~ \text{lognormal}(\mu, \sigma))$. We will also test whether a slightly more complex extension of this model, the \textit{shifted lognormal} model offers any improvement in model fit.

\subsubsection{Fitting the single feature search data}

We proceed in a similar fashion to section \ref{sec:fitting_D}, although this time, we fit each model three times using a (i) normal, (ii) lognormal, and (iii) shifted lognormal distribution. Please see Supplementary Materials for full details of out modelling approach, including prior predictive checks and model fit diagnostics. The three models were compared using leave-one-out cross validation, and the results showed that the shifted-lognormal model (illustrated in Figure \ref{fig:buetti2019_a1}) was given $100\%$ of the weight. Therefore, we will only use this model for the rest of the analysis. 


\begin{figure}
\centering
%\subfigure{\includegraphics[width=\textwidth]{../plots/buetti2019_bayesian_fits.pdf}}
\caption{The shaded regions show the model's predicted D values for each of the conditions in Expt 1 and Expt 3.}
...
\begin{figure}
\centering
%\subfigure{\includegraphics[width=\textwidth]{../plots/recreate_normal_fig_4.pdf}}
%\subfigure{\includegraphics[width=\textwidth]{../plots/recreate_log_normal_fig_4.pdf}}
