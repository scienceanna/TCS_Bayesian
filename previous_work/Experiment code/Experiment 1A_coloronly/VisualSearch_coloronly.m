...
%
%   This experiment uses a light blue target (that faces left or right; the 
...
%%%%%%%%%%%%%%%%%% INITIALIZATION %%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   KbName('UnifyKeyNames');
    ...
    % Output file
    fid=fopen(['VisualSearch_coloronly_' num2str(SbjID) '.out'],'a');  
...
%   green=[0,255,0];
    ...
    tsetsize=[1];     %number of light blue half circle
    dsetsize=[0,1,4,9,19,31]; %number of distractors in display (yellow, orange and blue half circles)
...
bright=imread('rbcircle.jpg'); %blue right circle
bleft=imread('lbcircle.jpg'); %blue left circle

%bl=Screen('MakeTexture',window,b);
%ye=Screen('MakeTexture',window,y);
yright=imread('rycircle.jpg'); %yellow triangle
yleft=imread('lycircle.jpg');%yellow left triangle

oright=imread('rocircle.jpg');%orange right circle
oleft=imread('locircle.jpg');%orange left circle

%og=Screen('MakeTexture',window,o);

% gr=Screen('MakeTexture',window,g);
br=Screen('MakeTexture',window,bright);
bl=Screen('MakeTexture',window,bleft);
yr=Screen('MakeTexture',window,yright);
yl=Screen('MakeTexture',window,yleft);
or=Screen('MakeTexture',window,oright);    
ol=Screen('MakeTexture',window,oleft);    
...
txt=[
    ...
    'Among these shapes there will always be one light blue semi-circle oriented \n\n' ...
    'with its arc to the left or the right of the vertical line.\n\n'...
    'Your task is to find the light blue semi-circle and identify which direction it is \n\n'...
...
%             end;
        ...
        %populate grid
        cnt2_l=0; %for loc(i)==2
        cnt2_r=0;
        cnt3_l=0; %for loc(i)==3
        cnt3_r=0;
        cnt4_l=0; %for loc(i)==4
        cnt4_r=0;
...
%                 tempy= cy +(round(rand(1)*jitter))+offset;
                ...
                if randnum<=.5
                    if cnt2_l<=2
                    Screen('DrawTexture',window, ol, [], [tempx tempy tempx+30 tempy+30]);
                    cnt2_l=cnt2_l+1;
                    else
                    Screen('DrawTexture',window, or, [], [tempx tempy tempx+30 tempy+30]);  
                    end
                else
                    if cnt2_r<=2
                    Screen('DrawTexture',window, or, [], [tempx tempy tempx+30 tempy+30]);
...
%                 tempy= cy +(round(rand(1)*jitter))+offset;
                ...
                if randnum<=.5
                    ...
                    Screen('DrawTexture',window, br, [], [tempx tempy tempx+30 tempy+30]);  
                    end
                else
                    if cnt3_r<=2
                    Screen('DrawTexture',window, br, [], [tempx tempy tempx+30 tempy+30]);
...
%                 tempy= cy +(round(rand(1)*jitter))+offset;
                ...
                if randnum<=.5
                    if cnt4_l<=2
                    Screen('DrawTexture',window, yl, [], [tempx tempy tempx+30 tempy+30]);
                    cnt4_l=cnt4_l+1;
                    else
                    Screen('DrawTexture',window, yr, [], [tempx tempy tempx+30 tempy+30]);  
                    end
                else
                    if cnt4_r<=2
                    Screen('DrawTexture',window, yr, [], [tempx tempy tempx+30 tempy+30]);
