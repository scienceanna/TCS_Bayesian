...
%%%%%%%%%%%%%%%%%% INITIALIZATION %%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   KbName('UnifyKeyNames');
    ...
    % Output file
    fid=fopen(['VisualSearch_TriTar_colorandshape_Comb3_' num2str(SbjID) '.out'],'a');  
...
y=imread('ycircle.png'); %yellow circle
%bl=Screen('MakeTexture',window,b);
or=imread('oRsemicir.png'); %right orange semi-circle
%ye=Screen('MakeTexture',window,y);
ol=imread('oLsemicir.png');%left orange semi-circle
b=imread('bdiamond.png');%blue diamond
%og=Screen('MakeTexture',window,o);

% gr=Screen('MakeTexture',window,g);
yl=Screen('MakeTexture',window,y);
olr=Screen('MakeTexture',window,or);
oll=Screen('MakeTexture',window,ol);
bl=Screen('MakeTexture',window,b);    
...
%                 tempy= cy +(round(rand(1)*jitter))+offset;
                ...
                if randnum<=.5
                    if cnt_l<=2
                    Screen('DrawTexture',window, oll, [], [tempx tempy tempx+30 tempy+30]);
                    cnt_l=cnt_l+1;
                    else
                    Screen('DrawTexture',window, olr, [], [tempx tempy tempx+30 tempy+30]);  
                    end
                else
                    if cnt_r<=2
                    Screen('DrawTexture',window, olr, [], [tempx tempy tempx+30 tempy+30]);
