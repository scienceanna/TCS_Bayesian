...
%%%%%%%%%%%%%%%%%% INITIALIZATION %%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   KbName('UnifyKeyNames');
    ...
    % Output file
    fid=fopen(['VisualSearch_TriTar_colorandshape_Comb2_' num2str(SbjID) '.out'],'a');  
...
o=imread('ocircle.png'); %orange circle
%bl=Screen('MakeTexture',window,b);
br=imread('bRsemicir.png'); %right blue semi-circle
%ye=Screen('MakeTexture',window,y);
bl=imread('bLsemicir.png');%left blue semi-circle
y=imread('ydiamond.png');%yellow diamond
%og=Screen('MakeTexture',window,o);

% gr=Screen('MakeTexture',window,g);
ol=Screen('MakeTexture',window,o);
blr=Screen('MakeTexture',window,br);
bll=Screen('MakeTexture',window,bl);
yl=Screen('MakeTexture',window,y);    
...
%                 tempy= cy +(round(rand(1)*jitter))+offset;
                ...
                if randnum<=.5
                    if cnt_l<=2
                    Screen('DrawTexture',window, bll, [], [tempx tempy tempx+30 tempy+30]);
                    cnt_l=cnt_l+1;
                    else
                    Screen('DrawTexture',window, blr, [], [tempx tempy tempx+30 tempy+30]);  
                    end
                else
                    if cnt_r<=2
                    Screen('DrawTexture',window, blr, [], [tempx tempy tempx+30 tempy+30]);
