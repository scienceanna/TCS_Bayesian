...
%
%   This experiment uses gray scale objects.  The target is a triangle 
%   (that faces left or right) and asks participants to respond to the direction of 
%   the target using a key press (left arrow and right arrow corresponding 
%   to the left and right directions). The displays are composed of homogenous 
%   gray scale distractors that vary in set size at six levels that always 
%   included a target (1 or 0 distractors, 2, 5, 10, 20, 32). 
%   Objects can be a gray diamond, triangle (facing left or right), circle.
%   
%   jitter&offset in this code is suppressed to adjust to grid_nc
...
%%%%%%%%%%%%%%%%%% INITIALIZATION %%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   KbName('UnifyKeyNames');
    ...
    % Output file
    fid=fopen(['VisualSearch_TriTar_shapeonly_' num2str(SbjID) '.out'],'a');  
...
b=imread('graycircle.jpg'); %gray circle
%bl=Screen('MakeTexture',window,b);
yr=imread('rlgraycircle.jpg'); %right gray semi-circle
%ye=Screen('MakeTexture',window,y);
yl=imread('llgraycircle.jpg');%left gray semi-circle
o=imread('graydiamond.jpg');%diamond diamond
...
rr=imread('grayRtri.jpg');
rrt=Screen('MakeTexture',window,rr);
%tid=0
rl=imread('grayLtri.jpg');
rlt=Screen('MakeTexture',window,rl);
%tid=1
...
txt=[
    '\n \n'...
    'In this experiment you will see a series of displays containing shapes.\n\n'...
    'Among these shapes there will always be one  semi-circle oriented \n\n' ...
    'with its arc to the left or the right of the vertical line.\n\n'...
    'Your task is to find the  semi-circle and identify which direction it is \n\n'...
    'oriented. If the arc is to the left of the vertical line, \n\n'...
    'you will respond with the left \n\n'...
...
for itrial=1:T_Trials
    Screen('DrawLine',window,green,Xcentre-9,Ycentre,Xcentre+9,Ycentre)
    Screen('DrawLine',window,green,Xcentre,Ycentre-9,Xcentre,Ycentre+9)
    %Screen('DrawLine',window,blue,Xcentre-9,Ycentre,Xcentre+9,Ycentre)
    %Screen('DrawLine',window,blue,Xcentre,Ycentre-9,Xcentre,Ycentre+9)
